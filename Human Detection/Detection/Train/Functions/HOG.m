...
image = double(image(((size(image,1)-128)/2)+1:128+((size(image,1)-128)/2),((size(image,2)-64)/2)+1:64+((size(image,2)-64)/2),:));
%% Computing Gradient Image
verticalFilter = [-1;0;1];
horizontalFilter = [-1,0,1];
xgradientImage = double(imfilter(image , verticalFilter,'same'));
ygradientImage = double(imfilter(image , horizontalFilter,'same'));
gradientMagnitudeImage = sqrt(double(xgradientImage.^2+ygradientImage.^2));
